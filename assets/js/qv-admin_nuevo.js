...
function initAutocomplete() {
    ...
    // Agregar hidden inputs al guardar post
	var postForm = document.getElementById("post");
	if (postForm) {
		postForm.addEventListener("submit", function(){
			crearHiddenInput("qv_distancia", document.getElementById("qv-distancia").textContent.replace(" km",""));
			crearHiddenInput("qv_importe", document.getElementById("qv-importe").textContent.replace("$",""));
		});
	}
}

function crearHiddenInput(name, value) {
	...
	service.getDistanceMatrix({
    	...
    	if (status === "OK") {
    		...
    		if (element.status === "OK") {
                var distanciaTexto = element.distance.text;
                var distanciaKm = element.distance.value / 1000;
